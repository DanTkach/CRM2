...
@login_required(login_url='login')
@allowed_users(allowed_roles=['admin'])
def arrearsCollection(request):
    contracts_list = Contract.objects.filter(status="Colectare")
    profile = [0, 0, 0, 0, 0]
    filter = [0, 0, 0, 1, 0]
    contracts = []
    for contract_data in contracts_list:
...
@login_required(login_url='login')
@allowed_users(allowed_roles=['admin'])
def arrearsSuing(request):
    contracts_list = Contract.objects.filter(status="Judecata")
    profile = [0, 0, 0, 0, 0]
    filter = [0, 0, 0, 0, 1]
    contracts = []
    for contract_data in contracts_list:
